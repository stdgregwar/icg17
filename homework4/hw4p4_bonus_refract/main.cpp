...
void Init(GLFWwindow* window) {
    ...
    GLuint tex_id;
    GLuint depth_id;
    std::tie(tex_id,depth_id) = framebuffer.Init(window_width,window_height);
    cube.Init(tex_id,depth_id);
...
void Display() {
    ...
    float r = 2.f;
    float t = -0.1*glfwGetTime();
    vec3 cam_pos(r*cos(t), r*sin(t), 2.0f);
    ...
    shinyfloor.Draw(view_projection);
    cube.Draw(view_projection,view);
}
...
int main(int argc, char *argv[]) {
    ...
    GLFWwindow* window = glfwCreateWindow(window_width, window_height, "Glass cube with ripples", NULL, NULL);
    if( !window ){
        glfwTerminate();
        exit(EXIT_FAILURE);
    }
